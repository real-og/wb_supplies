...
if __name__ == "__main__":
    while True:
        try:
            ...
            wb_fbs_stocks, warehouse_id = wb_api_helper.get_fbs_stocks(WB_TOKEN)
            time.sleep(2)
            ...
            for card in my_cards.json()['cards']:
                ...
                if vendor in config_io.get_value('AUTOSTOCK_EXCLUDE'):
                    continue
                ...
                if mode == "NOTIFICATION":
                    ...
                    if (int(fbw_amount) <= 3) and (int(fbs_amount) <= 3):
                        wb_api.add_product_to_fbs_warehouse(WB_TOKEN, warehouse_id, chrt_id, 100)
                        text = texts.autostock_added_fbs(vendor, fbs_amount, fbw_amount) 
                    elif (int(fbw_amount) > 3) and (int(fbs_amount) > 0):
                        wb_api.delete_product_from_fbs_warehouse(WB_TOKEN, warehouse_id, chrt_id)
