...
def export_supply_barcodes_to_excel(
    ...
    Количество рассчитывается С УЧЕТОМ КОЭФФИЦИЕНТА:
        round(shipment_qty * coefficient)
    ...
    """
    if coefficient < 0:
        raise ValueError("coefficient не может быть отрицательным")
    ...
    for item in sorted_result_items:
        base_shipment_qty = float(item.get("shipment_qty") or 0)
        shipment_qty = int(round(base_shipment_qty * coefficient))
