...
def export_supply_plan_to_excel(
    ...
    def _calc_qty_with_coefficient(shipment_qty: Any) -> int | None:
        if coefficient is None:
            return None
        return int(round(_safe_count_value(shipment_qty) * coefficient))

    def _calc_daily_sales_with_coefficient(daily_sales: Any) -> float | None:
        if coefficient is None:
            return None
        return _safe_count_value(daily_sales) * coefficient

    total_shipment_units_with_coefficient = None
    if coefficient is not None:
        total_shipment_units_with_coefficient = sum(
            _calc_qty_with_coefficient(item.get("shipment_qty")) or 0
            for item in items
        )
    ...
    meta_rows = [
        ...
        ("Количество товаров с учетом коэффициента", total_shipment_units_with_coefficient),
    ...
    network_headers = [
        ...
        "Продажи в день",
        "Ожидаемые продажи в день с учетом коэффициента",
        "На сколько дней хватит без рассматриваемой поставки",
        "На сколько дней хватит без рассматриваемой поставки с учетом коэффициента",
        "Количество в поставке",
        "Товаров с учетом коэффициента",
    ...
    for row_idx, item in enumerate(sorted_items, start=header_row_network + 1):
        ...
        network_daily_sales_with_coefficient = _calc_daily_sales_with_coefficient(network_daily_sales)
        shipment_qty = _safe_count_value(item.get("shipment_qty"))
        shipment_qty_with_coefficient = _calc_qty_with_coefficient(shipment_qty)
        ...
        row_values = [
            ...
            orders_14d_network,
            network_daily_sales,
            network_daily_sales_with_coefficient,
            None,  # formula
            None,  # formula
            shipment_qty,
            shipment_qty_with_coefficient,
            None,  # formula
        ]
        ...
        # I = без поставки, без коэффициента
        ws_network.cell(row=row_idx, column=9).value = (
            f'=IF(G{row_idx}=0,IF(E{row_idx}>0,"∞",""),E{row_idx}/G{row_idx})'
        )

        # J = без поставки, с коэффициентом
        if coefficient is not None:
            ws_network.cell(row=row_idx, column=10).value = (
                f'=IF(H{row_idx}=0,IF(E{row_idx}>0,"∞",""),E{row_idx}/H{row_idx})'
            )

        # M = после поставки, без коэффициента
        ws_network.cell(row=row_idx, column=13).value = (
            f'=IF(G{row_idx}=0,IF(E{row_idx}+K{row_idx}>0,"∞",""),(E{row_idx}+K{row_idx})/G{row_idx})'
        )

    for row_idx in range(header_row_network + 1, ws_network.max_row + 1):
        for col_idx in (3, 4, 5, 6, 11, 12):
            ws_network.cell(row=row_idx, column=col_idx).number_format = '#,##0'
        for col_idx in (7, 8, 9, 10, 13):
            ws_network.cell(row=row_idx, column=col_idx).number_format = '0.00'

    if ws_network.max_row >= header_row_network:
        ws_network.auto_filter.ref = f"A{header_row_network}:M{ws_network.max_row}"
    ...
    if best_warehouse:
        for row_idx, item in enumerate(sorted_items, start=2):
            ...
            daily_sales_with_coefficient = _calc_daily_sales_with_coefficient(daily_sales)
            shipment_qty = _safe_count_value(item.get("shipment_qty"))
            shipment_qty_with_coefficient = _calc_qty_with_coefficient(shipment_qty)
            ...
            # I = без поставки, без коэффициента
            ws_best_wh.cell(row=row_idx, column=9).value = (
                f'=IF(G{row_idx}=0,IF(E{row_idx}>0,"∞",""),E{row_idx}/G{row_idx})'
            )
            ...
            # M = после поставки, без коэффициента
            ws_best_wh.cell(row=row_idx, column=13).value = (
                f'=IF(G{row_idx}=0,IF(E{row_idx}+K{row_idx}>0,"∞",""),(E{row_idx}+K{row_idx})/G{row_idx})'
            )
        ...
        ws_best_wh.auto_filter.ref = f"A1:M{ws_best_wh.max_row}"
    else:
